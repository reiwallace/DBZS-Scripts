...
var counter = API.createCustomOverlay(0); // Countdown counter overlay
var counterFormat = "&1&l"
...
function timer(e) {
    ...
    if(id == 0) { // Start round
        if(activePlayer != null) {
            ...
            countDown = roundLength/20 - 1;
            setTimerOverlay(activePlayer, countDown); 
    ...
    } else if(id == 2) { // Round counter
        setTimerOverlay(activePlayer, countDown); 
        if(npc.timers.has(1) && npc.timers.ticks(1) < 60) {
...
function endRound(npc, doNext, outcomeText) { // End round stopping timers and reseting poses
    ...
    npc.timers.stop(2);
    resetPoses(npc);
    removeTimerOverlay(activePlayer);
    if(doNext) {
...
function newGame(npc, player) { // Start new game
    activePlayer = player;
    counter.addLine(2, 440, 19, 520, 19, 1, 1);
    counter.addLine(3, 440, 19, 440, 29, 1, 1);
    counter.addLine(4, 440, 29, 519, 29, 1, 1);
    counter.addLine(5, 519, 19, 519, 29, 1, 1);
    player.showCustomOverlay(counter);
...
function setTimerOverlay(target, value) {
    if(target != null) {
        if(value > 9) {
            counter.addLabel(1, counterFormat + "Remaining: " + value, 442, 20, 0, 0);
        } else {
            counter.addLabel(1, counterFormat + "Remaining: 0" + value, 442, 20, 0, 0);
        }
        counter.update(target);
    }
}

function removeTimerOverlay(target) { // Remove overlay items
    for(i = 0; i < 6; i++) {
        counter.removeComponent(i);
    }
    if(target != null) {
        counter.update(target);
    }
