...
var strikeSound = API.createSound("jinryuudragonbc:DBC2.strongpunch");
...
function timer(event)
{
  ...
  switch(event.id) {
    case(STRIKE_OF_REVELATION_TELEGRAPH):
      ...
      npc.setPosition(
        target.x + -targetLookVector.getXD(), 
        target.y, 
        target.z + -targetLookVector.getZD()
      );
      npc.setSpeed(0);
    ...
    case(STRIKE_OF_REVELATION):
      ...
      for(var i = 0; i < nearbyPlayers.length; i++) {
        ...
        // Sound
        strikeSound.setPosition(player.x, player.y, player.z);
        player.playSound(strikeSound);
